...
void SPI2_Inits(void)
{
	...
	SPI2handle.SPIConfig.SPI_SCLKSpeed = SPI_SCLK_SPEED_PRESCALER_32;
...
void Slave_GPIO_InterruptPinInit(void)
{
	...
	spiIntPin.GPIO_PinConfig.GPIO_PinMode = GPIO_MODE_IP_FET;
	spiIntPin.GPIO_PinConfig.GPIO_PinSpeed = GPIO_SPEED_LOW;
	spiIntPin.GPIO_PinConfig.GPIO_PinPuPdControl = GPIO_PIN_PU;

	GPIO_Init(&spiIntPin);

	GPIO_IRQPriorityConfig(IRQ_NUM_EXTI9_5,NVIC_IRQ_PRIORITY_NUM_15);
	...
	*/
	...
	while(1){
		...
		while(!rcvStop)
		{
			...
			while ( SPI_SendDataInterruptBased(&SPI2handle,&dummy,1) == SPI_BUSY_IN_TX);
			while ( SPI_ReceiveDataInterruptBased(&SPI2handle,&ReadByte,1) == SPI_BUSY_IN_RX );
		}
