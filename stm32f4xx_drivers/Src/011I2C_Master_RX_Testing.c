...
int main(void){
	...
	while(1){
		...
		commandCode = 0x51;
		I2C_MasterSendData(&I2C1Handle, &commandCode, 1, SLAVE_ADDRESS, I2C_SR_ENABLE);

		// Receive the number/length of bytes to be received from slave in variable 'length'
		I2C_MasterReceiveData(&I2C1Handle, &length, 1, SLAVE_ADDRESS, I2C_SR_ENABLE);
		...
		// Receive the number/length of bytes to be received from slave in variable 'length'
		I2C_MasterReceiveData(&I2C1Handle, receiveBuffer, length, SLAVE_ADDRESS, I2C_SR_DISABLE);
