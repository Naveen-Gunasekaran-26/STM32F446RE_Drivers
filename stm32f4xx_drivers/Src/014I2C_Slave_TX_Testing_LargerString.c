/*
 * 014I2C_Slave_TX_Testing_LargerString.c
 *
...
uint8_t txBuffer[] = "1_2_3_4_5_6_7_8_9_10_11_12_13_14_15_16_17_18_19_20_21_22_23_24_25_This_string_length_is_greater_than_32";
uint8_t txBufferLength;

uint8_t commandCode;
uint8_t bufferIndex = 0;
uint8_t byteCount = 0;
//uint32_t bufferIndex = 0; //This should be the actual one
//uint32_t byteCount = 0; //This should be the actual one
...
int main(void){
	...
	txBufferLength = strlen((char *)txBuffer);
...
void I2C_ApplicationEventCallback(I2C_Handle_t *pI2CHandle, uint8_t AppEvent){
	...
	if(AppEvent == I2C_EV_DATA_REQUEST){
		...
		if(commandCode == 0x51){
			...
			I2C_SlaveSendData(pI2CHandle->pI2Cx, ( ( txBufferLength >> ((byteCount%4) * 8) ) & 0xFF ) );
			byteCount++;
		}
		...
		// Invalidate if current commandCode != 0x52
		if(commandCode != 0x52)
			commandCode = 0xFF;

		byteCount = 0;

		//Slave concludes it sent all the bytes when bufferIndex reaches txBufferLength
		if(bufferIndex >= txBufferLength)
		{
			bufferIndex = 0;
			commandCode = 0xff;
		}
