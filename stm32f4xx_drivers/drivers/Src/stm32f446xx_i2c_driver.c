...
void I2C_SlaveSendData(I2C_RegDef_t *pI2Cx, uint8_t dataBuffer){
...
void I2C_EV_IRQHandling(I2C_Handle_t *pI2CHandle){
	...
	if( bit1State && bit2State && bit3State ){
		...
		if( pI2CHandle->pI2Cx->SR2 & (1 << I2C_SR2_MSL) ){
			...
			// Checking if the slave is in transmitter mode
			// TRA = 0 in receiver mode
			// TRA = 1 in transmitter mode
	...
	if( bit1State && bit2State && bit3State ){
		...
		if( pI2CHandle->pI2Cx->SR2 & (1 << I2C_SR2_MSL) ){
			...
			// Checking if the slave is in receiver mode
			// TRA = 0 in receiver mode
			// TRA = 1 in transmitter mode
			if( ! (pI2CHandle->pI2Cx->SR2 & (1 << I2C_SR2_TRA)) )
